"""
This file have
1) base RLModel class
	explanatory comments can find in Model class at train_sv.py
2) RlAgent that uses RLModel to read, write (normal, rollout) dialogue,
	and update function that uses policy gradient at the end of the dialogue with SVAgent
"""
...
class RLModel():
	...
	def _build_predictions(self):
		self.lang_softmax = tf.nn.softmax(self.lang_logit)
	...
	def _prepare_loss_rl(self):
		self.rewards_rl = tf.placeholder(shape=(None), dtype=tf.float32)
		self.words_rl = tf.placeholder(shape=(None), dtype=tf.int32)

		probs = self.lang_softmax[0]

		gather_indices = tf.range(tf.shape(probs)[0]) * tf.shape(probs)[1] + self.words_rl
		picked_action_probs = tf.gather(tf.reshape(probs, [-1]), gather_indices)

		losses = - tf.log(picked_action_probs) * self.rewards_rl
		self.loss_rl = tf.reduce_mean(losses, name="loss")

		self.train_op_rl = tf.train.AdamOptimizer(learning_rate=1e-4).minimize(self.loss_rl)
...
class RlAgent(object):
	...
	def write_normal(self, start="YOU:"):
		...
		self.mask.extend([0.] + [1.]*(len(words_collect)-1))
	...
	def update(self, agree, reward):
		...
		_, loss_rl = self.sess.run([self.model.train_op_rl, self.model.loss_rl], {
			self.model.ctx_count: self.count, 
			self.model.ctx_val: self.val,
			self.model.inpt: [self.words[:-1]],
			self.model.rewards_rl: rewards[1:],
			self.model.init_lang_state: self.model.lang_state_one,
			self.model.words_rl: self.words[1:]
		})
